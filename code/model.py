...
class NeuralCollaborativeFiltering(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        vector = torch.cat([gmf_product, mlp_output], dim=1)
        ratings =  self.output_logits(vector)
        return ratings
    ...
    def join_output_weights(self):
        W = torch.cat((self.model_blending*self.gmf_out.weight, (1-self.model_blending)*self.mlp_out.weight), dim=1)
        self.output_logits.weight.data = W
